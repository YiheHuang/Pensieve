...
可选 AI 会提取标题、人物、地点、主题和情绪。它不会生成正文摘要，也不会把你的表达改写成另一种故事，更不会修改记忆发生时间。时间默认采用首次点击“提取”时的北京时间，只有用户在高级选项或详情页中主动调整时才会改变。人物、地点与主题会在本地进一步标准化和去重。

### 🌊 沉浸式重新进入

在“沉浸”中用自然语言描述你想找的片刻，也可以按情绪、日期和媒体类型筛选。详情页与沉浸回放均可通过鼠标或左右方向键连续浏览相邻记忆；沉浸状态下切换不会浮回详情页，而是在同一片水下空间中柔和过渡。影像与文字消融在不规则流光、雾气和颜料入水般的背景中；纯文字记忆会自动占据完整画面，不保留空媒体区域。

### ✨ 在水面书写

提取、沉浸与长廊中的输入会在聚焦时唤醒柔和的内光与水纹，并在离开后归于平静。银蓝星光指针沿移动方向留下短暂微光，让书写与寻找都延续同一种安静的魔法语言；减少动态效果与高对比度模式也得到完整适配。
...
选择一段北京时间范围，Pensieve 会从情绪轨迹、人物与关系、地点与场景、主题事件及变化洞察等维度整理这一阶段。报告作为加密快照保存到档案库，并以全屏水下分页方式呈现；记忆数量、活跃日期与情绪占比会化作带有不同色彩的水下光谱，每个珍贵片段也能准确沉入对应的原记忆。
...
- `.pensieve` 备份使用独立密码加密，包含记忆、附件、时光回响档案及跨设备解锁所需的加密参数，并排除 AI API 密钥。
- 将备份迁入另一台电脑后，继续使用**来源记忆库原来的 PIN** 解锁；目标电脑初始化时设置的 PIN 不会覆盖来源记忆库。
