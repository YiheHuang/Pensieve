...
import { ArchiveRestore, Grid2X2, Heart, List, Search, Sparkles, Trash2 } from 'lucide-react'
...
export function TimelinePage() {
  ...
  const [view, setView] = useState<'timeline' | 'grid'>('timeline'); const [status, setStatus] = useState<'active' | 'trashed'>('active'); const [query, setQuery] = useState(''); const [favoriteOnly, setFavoriteOnly] = useState(false)
  ...
  return <div className="page timeline-page">
    ...
    {!shown.length ? <div className="no-results"><ArchiveRestore /><h3>{favoriteOnly ? t('还没有被点亮的珍贵记忆', 'No treasured memories yet') : t('这里暂时空空的', 'It is quiet here')}</h3><p>{favoriteOnly ? t('在记忆详情点击爱心，它就会来到这里。', 'Tap the heart in a memory to bring it here.') : status === 'trashed' ? t('被放回长廊的记忆会重新泛起微光。', 'Restored memories will glow in the gallery again.') : t('第一缕记忆正等着你存入。', 'Your first memory is waiting.')}</p></div> : view === 'grid' ? <div className="cards-grid timeline-grid">{shown.map(m => <MemoryCard key={m.id} memory={m} />)}</div> : <div className="timeline">{groups.map(([month, items]) => <section className="timeline-group" key={month}><div className="timeline-month"><span>{month}</span><i /></div><div className="timeline-items">{items?.map(m => <div className="timeline-row" key={m.id}><time><strong>{format(new Date(m.occurredAt), 'dd')}</strong><span>{format(new Date(m.occurredAt), 'EEEE', { locale })}</span></time><MemoryCard memory={m} featured />{status === 'trashed' && <button className="restore-button" onClick={async () => { await memoryRepository.restore(m.id); refetch() }}><ArchiveRestore /> {t('唤醒', 'Restore')}</button>}</div>)}</div></section>)}</div>}
  </div>
}
