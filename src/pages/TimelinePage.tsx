...
export function TimelinePage() {
  ...
  const purge = async (id: string, title: string) => {
    if (!window.confirm(t(`彻底删除“${title}”？正文与附件将一并消散。`, `Delete “${title}” permanently? Its text and attachments will be erased.`))) return
    await memoryRepository.purge(id)
    await refetch()
  }
  const trashActions = (memory: typeof shown[number]) => <div className="trash-actions"><button onClick={async () => { await memoryRepository.restore(memory.id); await refetch() }}><ArchiveRestore /> {t('唤醒', 'Restore')}</button><button className="purge-button" onClick={() => void purge(memory.id, memory.title)}><XCircle /> {t('彻底删除', 'Delete forever')}</button></div>
